...
class PhysicalDeviceController extends VoyagerBaseController
{
    ...
    public function ingRackOfDevice(Request $request, $deviceId)
    {

        $slug = "ingredient-racks";
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($orderBy && (in_array($orderBy, $dataType->fields()) || !empty($row))) {
                ...
                $dataTypeContent = call_user_func([
                    $query->where('fr_devices','=',$deviceId)->orderBy($orderBy, $querySortOrder),
                    $getter,
                ]);
            } elseif ($model->timestamps) {
                $dataTypeContent = call_user_func([$query->where('fr_devices','=',$deviceId)->latest($model::CREATED_AT), $getter]);
            } else {
                $dataTypeContent = call_user_func([$query->where('fr_devices','=',$deviceId)->orderBy($model->getKeyName(), 'DESC'), $getter]);
            }
            ...
            // If Model doesn't exist, get data from table name
            $dataTypeContent = call_user_func([DB::table($dataType->name)->where('fr_devices','=',$deviceId), $getter]);
            $model = false;
        }
    ...
    public function ingRackOfDeviceAvailable(Request $request, $deviceId){
        $slug="physical-devices";
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        $physicalDevice=PhysicalDevice::findOrFail($deviceId);
        $physicalDevice->setAttribute('israck', 2);
    ...
    {
        $active_tab=$request['active_tab']? $request['active_tab'] : 'device';
        $slug = $this->getSlug($request);
        ...
        $ingData=$this->ingRackOfDevice($request, $id);

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted','active_tab','ingData'));
    }
    ...
    public function editInDetail(Request $request, $id){
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isView','active_tab','ingData'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->israck==1){
            $this->destroyIngRackOfDevice($id);
        }
        // Delete Images
    ...
    public function updateIndetail(Request $request, $id)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted','active_tab','ingData'))->with([
            'message' => __('voyager::generic.successfully_updated') . " {$dataType->getTranslatedAttribute('display_name_singular')}",
            'alert-type' => 'success',
        ]);

    }
    public function storeIngRack(Request $request, $fr_device_id)
    {
        $slug="ingredient-racks";
    ...
    public function store(Request $request)
    {
        ...
        if($request->israck==1){
            $this->destroyIngRackOfDevice($data->id);
        }
    ...
    public function destroyIngRackOfDevice($deviceId){
        $slug = "ingredient-racks";

        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();

        $ids=DB::table('ingredient_racks')->select('id')->where('fr_devices', $deviceId )->where('deleted_at', '=',null)->pluck('id')->all();
