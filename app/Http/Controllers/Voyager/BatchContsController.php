...
class BatchContsController
{
    public function getActivate(Request $request) {
        $data = DB::table('production_activate')->select('*')->where("id", "=", 1)
            ->first();
        $activate = $data->activate;
        echo json_encode($activate);
    }
    public function updateActivate(Request $request) {
        $data = DB::table('production_activate')->update([ 'activate' => 0]);
    ...
    public function index(Request $request)
    {
        ...
        $view = 'voyager::production-orders.browse';
